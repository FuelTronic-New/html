...
$mytime = \Carbon\Carbon::now();
 ...
 $test_date=(string)$newdate;
			...
			//		$daid_fuel_count=count($daily_fuel_decode)-1;
					...
					$dashborad_tank_level_updated = DashboardTankLevel::select('TransactionDate','RunningTotal')->where('site_id', $site->id)->where('TankId', $tank->id)->orderBy('TransactionDate','DESC')->take(1)->get();
					...
					$tempCD['graph']['TransactionLastUpdate'] = $daily_cron_update_decode[0]['TransactionDate'];
						...
						if(isset($daily_cron_update_decode[0]['TransactionDate'])){
					$tempCD['graph']['RunningTotal'] = $daily_cron_update_decode[0]['RunningTotal'];
					}
					else{
						$tempCD['graph']['RunningTotal'] = '';
					}
					
				/* this code use for get last updated cron for dashboard end */
					//$tempCD['graph']['RunningTotal'] = number_format($daily_cron_update_decode[0]['RunningTotal'],2);
...
//			$returnData = array ( 'status' => 'success', 'data'=> ['sites'=>$Data,'xaxis'=>$xaxis, 'tankData' => $finalData], 'code' => 200);
	...
	{
			//date_default_timezone_set('Africa/Johannesburg');
			echo $dash_cur=date("Y-m-d H:i:s");
			die('here');
	}
